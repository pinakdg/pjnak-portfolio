export default function About(){
  return (
    <section id="about" className="max-w-6xl mx-auto px-4 py-12">
      <h2 className="text-2xl md:text-3xl font-semibold mb-4">about</h2>
      <p className="text-gray-300">
        i like computers. i enjoy tinkering with software and hardware. 
        i am passionate about system level programming and secure coding. 
        i have experience with arch linux, kali linux, windows and mac-os.
        i daily drive macos, kali for pen-testing and fedora for development.
        i am proficient in c, java, rust, python and bash/shell scripting. 
        i have a keen interest in malware analysis and reverse engineering. 
        i also enjoy participating in ctf competitions and contributing to open source projects.
        i enjoy hack the box labs and software development in free time.
        outside of tech, i play the guitar, practice mixed martial arts, am into video games and everything related to motorcycles.
        i enjoy learning new things and challenging myself, be it tech, music, video games or mechanical know-how for tinkering with motorised vehicles.
      </p>
    </section>
  )
}